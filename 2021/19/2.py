...
def process(data):
    ...
    while queue:
        ...
        if status:
            ...
            scanner_diffs.append(fixed_diff_coordinates)
        else:
            queue.append(scanner_id)
    manhattan = lambda p1, p2: sum(abs(a - b) for a, b in zip(p1, p2))
    return max(manhattan(p1, p2) for p1 in scanner_diffs for p2 in scanner_diffs)
