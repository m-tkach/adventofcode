...
def go(hallway, rooms, energy, energy_limit):
    ...
    for item, src_position in hallway.gen_actionable_items():
        if isinstance(item, Amphipod):
            ...
            if is_moved:
                actions.append((move_energy, hallway.move_to_room, hallway.unmove_to_room_DANGER, (src_position, room)))
            ...
            if amphipod and not amphipod.is_completed():
                ...
                if is_moved:
                    actions.append((move_energy, hallway.move_between_rooms, hallway.unmove_between_rooms_DANGER, (item, rooms[amphipod.get_room_index()])))
    ...
    for move_energy, apply_f, withdraw_f, args in sorted(actions, key=lambda i: i[0]):
        if move_energy >= result_energy:
            break
