...
def gen_next_sand_position(points, sand_x, sand_y, y_limit):
    if sand_y + 1 >= y_limit:
        return None
    for dx in 0, -1, 1:
...
def process(data):
    ...
    y_limit = max(map(itemgetter(1), points)) + 2
    sand_count = 0
    sand_x, sand_y = gen_new_sand_point()
    while gen_new_sand_point() not in points:
        next_sand_point = gen_next_sand_position(points, sand_x, sand_y, y_limit)
        if next_sand_point:
